# Pontos de Interesse - Serras Gaúchas 2026

## Mapa de Regiões

```
    GOIÂNIA
       ↓ (380 km)
    UBERABA
       ↓ (360 km)
    OURINHOS
       ↓ (350 km)
┌─────────────────────┐
│   CAMPOS GERAIS     │ ← Vila Velha, Buraco do Padre
│   (Ponta Grossa)    │
└─────────────────────┘
       ↓ (340 km)
┌─────────────────────┐
│  PLANALTO SERRANO   │ ← Lages, Pedras Brancas
│      (Lages)        │
└─────────────────────┘
       ↓ (110 km)
┌─────────────────────┐
│ SERRA DO CORVO      │ ← Morro da Igreja, Pedra Furada
│ BRANCO (Urubici)    │
└─────────────────────┘
       ↓ (50 km)
┌─────────────────────┐
│ SERRA DO RIO        │ ← 284 curvas, mirantes épicos
│ DO RASTRO           │
└─────────────────────┘
       ↓ (80 km)
┌─────────────────────┐
│   SÃO JOAQUIM       │ ← Vinícolas de altitude, frio
│ (Cidade mais fria)  │
└─────────────────────┘
       ↓ (180 km)
┌─────────────────────┐
│     CÂNIONS         │ ← Itaimbezinho, Fortaleza
│ (Cambará do Sul)    │
└─────────────────────┘
       ↓ (180 km)
┌─────────────────────┐
│ VALE DOS VINHEDOS   │ ← Vinícolas, gastronomia
│ (Bento Gonçalves)   │
└─────────────────────┘
       ↓ (280 km)
┌─────────────────────┐
│  LITORAL SC (NOVO)  │ ← Teleférico, praias, surfe
│ (Balneário Camboriú)│
└─────────────────────┘
       ↓ (60 km)
┌─────────────────────┐
│    BOMBINHAS        │ ← 39 praias, snorkel
│   (Península)       │
└─────────────────────┘
       ↓ (100 km)
┌─────────────────────┐
│ GUARDA DO EMBAÚ     │ ← Reserva Mundial de Surfe
│    (Palhoça)        │
└─────────────────────┘
       ↓ (230 km)
┌─────────────────────┐
│    CURITIBA         │ ← Serra Verde Express
│  (Capital do PR)    │
└─────────────────────┘
       ↓ (750 km)
    GOIÂNIA
```

---

## Campos Gerais - PR

### Parque Estadual de Vila Velha

| Informação | Detalhe |
|------------|---------|
| **Localização** | 25 km de Ponta Grossa |
| **Ingresso** | R$ 30-50 |
| **Horário** | 08:00-17:00 |
| **Tempo de visita** | 3-4 horas |

**Atrações:**
- **Arenitos** - Formações rochosas de 300 milhões de anos
- **Furnas** - Crateras gigantes com lagos
- **Lagoa Dourada** - Lago cristalino

**Destaques para fotos:**
- Taça (formação mais famosa)
- Camelo
- Bota

---

### Buraco do Padre

| Informação | Detalhe |
|------------|---------|
| **Localização** | Ponta Grossa |
| **Ingresso** | R$ 20-30 |
| **Tempo de visita** | 2 horas |

**O que ver:**
- Furna com 30m de diâmetro
- Cachoeira no interior
- Trilha fácil

---

### Capão da Onça

| Informação | Detalhe |
|------------|---------|
| **Localização** | 30 km de Ponta Grossa |
| **Ingresso** | R$ 15-25 |

**Atrações:**
- Conjunto de cachoeiras
- Trilhas ecológicas
- Piscinas naturais

---

## Serras Catarinenses

### Morro da Igreja

| Informação | Detalhe |
|------------|---------|
| **Altitude** | 1.822m (ponto mais alto de SC) |
| **Localização** | Entre Urubici e Bom Jardim |
| **Acesso** | Estrada de terra (4x4 recomendado) |
| **Melhor horário** | Nascer do sol |

**Destaques:**
- Estação meteorológica do INPE
- Vista 360° do planalto
- Geadas frequentes mesmo no verão

---

### Serra do Corvo Branco

| Informação | Detalhe |
|------------|---------|
| **Altitude máxima** | 1.140m |
| **Extensão** | 8 km |
| **Acesso** | Parque Altos do Corvo Branco |
| **Ingresso** | R$ 80/pessoa |

**ATENÇÃO:** Lado Grão-Pará fechado até março/2026

**Mirantes:**
- Mirante principal (estacionamento)
- Mirante da Pedra Furada
- Vista para o Vale do Rio Canoas

---

### Pedra Furada

| Informação | Detalhe |
|------------|---------|
| **Localização** | Urubici |
| **Trilha** | 1,5 km (fácil) |
| **Ingresso** | R$ 15-20 |

**O que ver:**
- Formação rochosa com furo natural
- Cartão postal de Urubici
- Melhor luz: final da tarde

---

### Cascata Véu de Noiva

| Informação | Detalhe |
|------------|---------|
| **Localização** | Urubici |
| **Altura** | 62m |
| **Trilha** | 800m |

---

### Serra do Rio do Rastro

| Informação | Detalhe |
|------------|---------|
| **Desnível** | 1.400m |
| **Curvas** | 284 |
| **Extensão** | 32 km |
| **Rodovia** | SC-438 |

**Mirantes obrigatórios:**
1. **Mirante da Serra** - Topo (vista completa)
2. **Mirante das Curvas** - Meio do trajeto
3. **Mirante do Vale** - Base

**Dicas:**
- Faça a DESCIDA de manhã (menos neblina)
- Pare nos mirantes para fotos
- Cuidado com caminhões

---

### São Joaquim - Vinícolas de Altitude

| Vinícola | Especialidade | Ingresso |
|----------|--------------|----------|
| **Villa Francioni** | Espumantes, estrutura completa | R$ 60-80 |
| **Villaggio Grando** | Vinhos finos | R$ 40-60 |
| **Abreu Garcia** | Pinot Noir | R$ 50-70 |
| **Pericó** | Vinhos autorais | R$ 40-60 |

**Diferencial:** Vinhos produzidos a 1.400m de altitude - únicos no Brasil

---

## Cânions - RS

### Parque Nacional de Aparados da Serra (Itaimbezinho)

| Informação | Detalhe |
|------------|---------|
| **Profundidade** | 720m |
| **Extensão** | 5,8 km |
| **Ingresso** | R$ 96,90/pessoa (2025) |
| **Validade** | 7 dias (ambos parques) |
| **Horário** | 08:00-17:00 |

**Trilhas:**
| Trilha | Distância | Dificuldade | Tempo |
|--------|-----------|-------------|-------|
| Vértice | 1,4 km | Fácil | 1h |
| Cotovelo | 6 km | Moderada | 3h |
| Rio do Boi | 8 km | Difícil | 6h (guia obrigatório) |

**Melhor mirante:** Trilha do Cotovelo - vista completa do cânion

---

### Parque Nacional da Serra Geral (Fortaleza)

| Informação | Detalhe |
|------------|---------|
| **Profundidade** | 900m |
| **Ingresso** | Mesmo do Itaimbezinho |
| **Distância** | 30 km de Cambará |

**Destaques:**
- Vista mais dramática que Itaimbezinho
- Cachoeira do Tigre Preto (visível do mirante)
- Pedra do Segredo

**ATENÇÃO:** Tirolesa fechada desde maio/2024

---

## Serra Gaúcha

### Gramado

| Atração | Tipo | Valor |
|---------|------|-------|
| **Mini Mundo** | Miniaturas | R$ 60-80 |
| **Snowland** | Neve artificial | R$ 180-250 |
| **Lago Negro** | Passeio gratuito | Gratuito |
| **Rua Coberta** | Compras/gastronomia | Gratuito |
| **Mundo de Chocolate** | Fábrica | R$ 40-60 |

**Destaques:**
- Centro histórico (arquitetura bávara)
- Chocolates (Caracol, Prawer, Lugano)
- Fondue à noite

---

### Canela

| Atração | Tipo | Valor |
|---------|------|-------|
| **Cascata do Caracol** | Cachoeira 131m | R$ 20-30 |
| **Parque do Caracol** | Trilhas | R$ 40-50 |
| **Catedral de Pedra** | Igreja gótica | Gratuito |
| **Parque da Ferradura** | Mirante | R$ 15-25 |
| **Teleférico** | Vista aérea | R$ 70-90 |

---

## Vale dos Vinhedos

### Vinícolas Recomendadas

| Vinícola | Destaque | Tour + Degustação |
|----------|----------|-------------------|
| **Casa Valduga** | Estrutura completa, restaurante | R$ 80-120 |
| **Miolo** | Maior produtora, vinhos acessíveis | R$ 60-90 |
| **Pizzato** | Melhores tintos, prêmios | R$ 70-100 |
| **Don Giovanni** | Vinhos autorais, intimista | R$ 60-80 |
| **Cave Geisse** | Espumantes método tradicional | R$ 70-100 |
| **Aurora** | Cooperativa centenária | R$ 40-60 |
| **Salton** | Tradição, variedade | R$ 50-80 |

**Rota sugerida (1 dia):**
1. Cave Geisse (manhã) - espumantes
2. Almoço na Casa Valduga
3. Miolo (tarde) - estrutura
4. Pizzato (final tarde) - tintos

---

### Maria Fumaça

| Informação | Detalhe |
|------------|---------|
| **Trajeto** | Bento Gonçalves - Garibaldi |
| **Duração** | 2h30 |
| **Valor** | R$ 200-300/pessoa |
| **Inclui** | Degustação, show |

---

## Litoral Catarinense

### Balneário Camboriú

| Informação | Detalhe |
|------------|---------|
| **Localização** | Litoral norte de SC |
| **Apelido** | "Dubai Brasileira" |
| **Destaque** | Prédios altos + praias |

**Atrações principais:**

| Atração | Tipo | Valor |
|---------|------|-------|
| **Teleférico Unipraias** | Transporte aéreo | R$ 70-100 |
| **Cristo Luz** | Monumento | R$ 30-50 |
| **Rodovia Interpraias** | Estrada cênica | Gratuito |
| **Parque Unipraias** | Aventura | R$ 50-80 |

**Teleférico Unipraias:**

| Informação | Detalhe |
|------------|---------|
| **Único no Brasil** | Liga duas praias pelo ar |
| **Trajeto** | Barra Sul → Laranjeiras |
| **Horário** | 09h-19h |
| **Duração** | ~3 minutos cada trecho |
| **Vista** | Panorâmica 360° da costa |

**Rodovia Interpraias:**
- Estrada cênica costeira (SC-412)
- Praias: Laranjeiras, Taquaras, Pinho, Estaleiro, Estaleirinho
- Mirantes com vistas incríveis do mar
- Curvas técnicas - ótimo para moto!

**Praias de Balneário:**
| Praia | Característica |
|-------|----------------|
| **Central** | Urbana, infraestrutura |
| **Barra Sul** | Partida teleférico |
| **Laranjeiras** | Chegada teleférico, calma |
| **Praia Brava** | Surfe, jovem |
| **Taquaras** | Família, sossegada |

---

### Bombinhas

| Informação | Detalhe |
|------------|---------|
| **Localização** | Península, 70km de Florianópolis |
| **Praias** | 39 praias na península |
| **TPA (taxa)** | R$ 3,50 para motos |
| **Acesso** | Via Porto Belo |

**Destaques:**

| Praia | Característica | Ideal para |
|-------|----------------|------------|
| **Sepultura** | Águas cristalinas | Snorkel, mergulho |
| **Bombinhas** | Principal, infraestrutura | Famílias |
| **Bombas** | Ondas | Surfe |
| **Zimbros** | Vila de pescadores | Gastronomia, tranquilidade |
| **Mariscal** | Extensa, dunas | Caminhada |
| **Lagoinha** | Pequena, preservada | Fotos |

**Mirantes e trilhas:**

| Local | Tipo | Tempo |
|-------|------|-------|
| **Morro do Macaco** | Trilha + mirante | 1h30 ida |
| **Costão da Sepultura** | Trilha curta | 30 min |
| **Mirante Eco 360** | Plataforma | 15 min caminhada |

**Dicas Bombinhas:**
- TPA obrigatória na entrada (R$ 3,50 moto)
- Estrada de acesso com curvas - divertida!
- Alta temporada: chegar cedo (8h)
- Sepultura: levar máscara de snorkel

---

### Guarda do Embaú

| Informação | Detalhe |
|------------|---------|
| **Localização** | 46km sul de Florianópolis |
| **Município** | Palhoça |
| **Status** | 9ª Reserva Mundial de Surfe |
| **Destaque** | Primeira do Brasil |

**O que torna especial:**
- Vilarejo preservado, clima rústico
- Atravessia de canoa pelo Rio da Madre
- Ondas consistentes para surfe
- Paisagem de Mata Atlântica
- Comunidade local acolhedora

**Como funciona o acesso:**
1. Estacionar a moto no vilarejo
2. Atravessar o Rio da Madre de canoa (R$ 5-10)
3. Caminhada pela praia

**Atrações:**

| Local | Característica |
|-------|----------------|
| **Praia da Guarda** | Principal, surfe |
| **Rio da Madre** | Travessia de canoa |
| **Trilha da Pedra do Urubu** | Vista panorâmica |
| **Vila** | Restaurantes, artesanato |

**Dicas:**
- Melhor época surfe: outono/inverno
- Janeiro: mar mais calmo, bom para banho
- Almoço: peixes frescos nos restaurantes locais
- Estrutura simples - leve água e protetor solar

---

### Serra Verde Express (Curitiba - Morretes)

| Informação | Detalhe |
|------------|---------|
| **Tipo** | Trem turístico histórico |
| **Trajeto** | Curitiba → Morretes |
| **Distância** | 110 km de trilhos |
| **Duração** | 4h15 (descida) |
| **Paisagem** | Serra do Mar, Mata Atlântica |

**Classes disponíveis:**

| Classe | Preço | Característica |
|--------|-------|----------------|
| **Econômica** | R$ 35-50 | Básica |
| **Turística** | R$ 80-100 | Recomendada |
| **Boutique** | R$ 130-150 | Confortável |
| **Luxo** | R$ 150-180 | Premium |

**Horários:**
| Sentido | Saída | Chegada |
|---------|-------|---------|
| Curitiba → Morretes | 08:30 | 12:45 |
| Morretes → Curitiba | 15:00 | 18:30 |

**Destaques do trajeto:**
- Viaduto Carvalho (curva sobre abismo)
- Ponte São João (engenharia histórica)
- Véu de Noiva (cachoeira visível do trem)
- Mata Atlântica preservada

**Morretes - O que fazer:**

| Atração | Tipo |
|---------|------|
| **Barreado** | Prato típico (carne 20h cozida) |
| **Centro histórico** | Casarões coloniais |
| **Rio Nhundiaquara** | Passeio de barco |
| **Feira de artesanato** | Souvenires locais |

**Retorno:**
- Van/ônibus: ~R$ 50 (1h30)
- Trem volta: comprar passagem separada
- Estrada da Graciosa: cênica mas demorada

**Dicas importantes:**
- Reservar antecipado: serraverdeexpress.com.br
- Sentar lado DIREITO (melhor vista descendo)
- Levar câmera/GoPro
- Jaqueta (ar-condicionado frio)

---

## Rota Romântica

### Nova Petrópolis

| Atração | Tipo | Valor |
|---------|------|-------|
| **Parque Aldeia do Imigrante** | Cultura alemã | R$ 20-30 |
| **Labirinto Verde** | Jardim | R$ 25-35 |
| **Praça das Flores** | Passeio | Gratuito |

**Gastronomia:**
- Café colonial (Opa's Kaffeehaus)
- Cucas e tortas alemãs
- Cervejarias artesanais

---

### Outras Cidades da Rota Romântica

| Cidade | Destaque | Distância de Nova Petrópolis |
|--------|----------|------------------------------|
| **São Leopoldo** | Museu da Imigração | 60 km |
| **Novo Hamburgo** | Comércio calçados | 50 km |
| **Ivoti** | Orquídeas | 35 km |
| **Dois Irmãos** | Cervejarias | 40 km |
| **Morro Reuter** | Vista panorâmica | 25 km |
| **Picada Café** | Artesanato em madeira | 20 km |

---

## Cachoeiras na Rota

| Nome | Localização | Altura | Acesso |
|------|-------------|--------|--------|
| Véu de Noiva | Urubici | 62m | Fácil |
| Avencal | Urubici | 100m | Trilha 2km |
| Cascata do Caracol | Canela | 131m | Parque |
| Cachoeira da Mariquinha | Ponta Grossa | 30m | Fácil |
| Tigre Preto | Cambará (vista) | 300m | Mirante Fortaleza |

---

## Mirantes Imperdíveis

| Mirante | Localização | O que ver |
|---------|-------------|-----------|
| **Morro da Igreja** | Urubici | Planalto 360° |
| **Serra do Rio do Rastro** | SC-438 | Curvas dramáticas |
| **Corvo Branco** | Urubici | Vale do Rio Canoas |
| **Fortaleza** | Cambará | Cânion + cachoeira |
| **Cotovelo** | Itaimbezinho | Cânion completo |
| **Caracol** | Canela | Cachoeira 131m |

---

## Melhor Época para Cada Atração

| Atração | Jan-Fev | Mai-Jul | Set-Nov |
|---------|---------|---------|---------|
| Serra Rio do Rastro | Bom | Neblina | Bom |
| Cânions | Bom | Frio | Bom |
| Vinícolas | Bom | Vindima | Bom |
| Urubici | Quente | Geada/neve | Ameno |
| Gramado | Lotado | Frio agradável | Florido |
| **Balneário Camboriú** | **Lotado, quente** | Vazio, frio | Agradável |
| **Bombinhas** | **Lotado, mar calmo** | Vazio | Bom |
| **Guarda do Embaú** | Mar calmo | Ondas! | Bom |
| **Serra Verde Express** | Bom | Frio, neblina | Bom |

---

## Tempo Recomendado por Região

| Região | Dias Ideais | Mínimo |
|--------|-------------|--------|
| Campos Gerais | 2 | 1 |
| Urubici/Corvo Branco | 2 | 1 |
| Rio do Rastro | 1 | 0.5 |
| São Joaquim | 2 | 1 |
| Cânions | 2 | 1 |
| Gramado/Canela | 2 | 1 |
| Vale dos Vinhedos | 2 | 1 |
| **Balneário Camboriú** | **2** | **1** |
| **Bombinhas** | **2** | **0.5** |
| **Guarda do Embaú** | **1** | **0.5** |
| **Curitiba + Serra Verde** | **2** | **1** |
| Rota Romântica | 1 | 0.5 |

---

*Valores de ingressos são estimativas para 2025/2026. Confirme antes de visitar.*

*Última atualização: Janeiro/2026 - Adicionado Litoral SC e Serra Verde Express*
